...
@section("content")
    <form action="{{route('dashboard.accountstype.store')}}" method="post">
        @csrf
    <div class="intro-y box p-5">
        <div class="text-2xl text-center">Please Add  Collection And Expences Type</div>
        {{error_message($errors)}}
        {{success_message()}}
        <div>
            <label>Collection And Expences Type:</label>
            <input type="text" name="name" class="input w-full border mt-2"  placeholder="Input Collection And Expences Type">
        </div>
        <br>
        <div class="mt-3">
            <label>Active Status:</label>
            <div id="packageStatus" class="mt-2 text-right">
                <input  type="checkbox" name="status" checked class="input input--switch border ">
            </div>
        </div>

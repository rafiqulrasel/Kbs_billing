...
@section("content")
    ...
    <div class="container w-full md:w-full xl:w-full  mx-auto px-2">
        ...
        <h1 class="font-sans font-bold break-normal text-indigo-500 px-2 py-8 text-xl md:text-2xl">
           All Collections and Expences Type
        </h1>
        {{btnAddNew('Add New Type','dashboard.accountstype.create')}}
        <!--Card-->
        <div id='recipients' class="mt-12 lg:mt-0 rounded shadow bg-white">
            <table id="example">
                ...
                <tr>
                    ...
                    <th data-priority="2">Collections and Expences Type</th>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('#example').DataTable({
                ...
                ajax: "{{ route('dashboard.accountstype.index') }}",
                columns: [
...
<script>
    function deleteUser(package) {
        ...
        if (confirm('Delete this Collection And Expences Type?')) {
            $.ajax({
                type: "DELETE",
                url: 'accountstype/' + id, //resource
                cache: false,
                data:{
                    _token:'{{ csrf_token() }}'
                },
                success: function(affectedRows) {
                    ...
                    if (affectedRows > 0) window.location = "{{route('dashboard.accountstype.index')}}";
                }
            });
        }
    }
